...
function Product_by_catagory(){
    ...
    return (
        <>
        {
            ...
            <div  margin={"auto"} width={"95%"} height={"auto"} mt={18} bgcolor={"white"} display={"flex"} flexDirection={"column"} gap={3} padding={2} overflow={"hidden"}> 
        {products.map((product)=>{
        return<>
        
       
                <div key={product._id} display={"flex"} justifyContent={"flex-start"} alignItems={"center"} gap={6} boxShadow={"2px 2px 2px #949494"} minHeight={250} pb={2} sx={{textDecoration:"none"}} component={Link} to={`/product/${product.product_catagory}/${product._id}`}> 
                        <div>
                            <img src={product.product_img} alt="" width={250} height={250} style={{objectFit:"contain"}}/>
                        </div>
                        <div display={"flex"} flexDirection={"column"} justifyContent={"center"} alignItems={"flex-start"} gap={2}> 
